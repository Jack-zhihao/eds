<template>
    <div class="chart-warp" id="rainLevelChartWarp">
        <p class="title">雨量监测</p>
        <div class="rainLevelChart" id="rainLevelChart"></div>
    </div>
</template>
<script setup>
...
const handleChartInit = () => {
    ...
    const option = {
        ...
        series: [
            ...
            areaStyle: {
                color: new echarts.graphic.LinearGradient(
                    0, 0, 0, 1,
                    [
                    { offset: 0, color:'rgba(33, 119, 249, 0.75)' },
                    { offset: 1, color: 'rgba(33, 119, 249, 0.05)' }
                    ]
                )
            }
            }
     ...
     setTimeout(() => {
        const rainLevelChartWarp = document.getElementById('rainLevelChartWarp');
        const rainLevelChart = document.getElementById('rainLevelChart');
        rainLevelChart.style.width = (rainLevelChartWarp.offsetWidth - 32) + 'px'
        myChart = echarts.init(rainLevelChart);
